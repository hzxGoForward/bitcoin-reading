 ...
 * merkleblock message, cmpctblock message, or notfound message.
 * 
 ...
 * 一些spv节点并不希望获取到整个区块以及相关交易,而只想获取区块头和一些自己关心的交易,对于其他交易并不关心
 * 因此,这些spv节点会向全节点发送filterload信息,即将自己关心的交易id压缩在一个布隆过滤器中,全节点向他们发送区块
 * 时,过滤掉不在布隆过滤器中的交易,只发送在其中的交易,以节省带宽
 * a merkleblock message is a block header, plus a part of a merkle tree which can be used to 
 * extract identifying information for transactions that matched the filter and prove that the 
 * matching transaction data really did appear in the solved block.
 */
