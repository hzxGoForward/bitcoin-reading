...
struct QueuedBlock {
    uint256 hash;
    const CBlockIndex* pindex;                              //!< Optional.包含每个区块的一些索引信息,hash,tx_count,height etc.
...
// atomic object can not be copyd or moved
// it can be shared by multi thread savely.
...
struct CNodeState {
    ...
    //! Length of current-streak of unconnecting headers announcements, 啥意思?
...
void RequestTx(CNodeState* state, const uint256& txid, std::chrono::microseconds current_time) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (peer_download_state.m_tx_announced.size() >= MAX_PEER_TX_ANNOUNCEMENTS ||    //peer 声明的交易数量太大,放弃
        peer_download_state.m_tx_process_time.size() >= MAX_PEER_TX_ANNOUNCEMENTS || // 向peer 请求的交易数量超过规定
