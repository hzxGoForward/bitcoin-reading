...
//
/**
 * hzx fDiscover表示是否发现自己的ip地址
 * fListen 表示是否支持接收inbound peer
 * g_relay_txes表示是否支持交易转发,默认支持交易转发
 * **/
bool fDiscover = true;
bool fListen = true;
bool g_relay_txes = !DEFAULT_BLOCKSONLY;
CCriticalSection cs_mapLocalHost;
std::map<CNetAddr, LocalServiceInfo> mapLocalHost GUARDED_BY(cs_mapLocalHost); // 记录本地的主机ip地址?
...
/*** hzx 和addrConnect建立连接
// pszDest指的是域名,每个域名对应多个ip地址,
// 如果是pszDest是域名,则随机选择所属的ip地址
// resolved中随机选择一个IP地址赋值给addrConnect
*/
...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char* pszDest, bool fCountFailure, bool manual_connection, bool block_relay_only)
{
    ...
    if (pszDest) {
        ...
        if (Lookup(pszDest, resolved, default_port, fNameLookup && !HaveNameProxy(), 256) && !resolved.empty()) {
            // hzx 从resolved中随机提取一个,赋值给addrConnect
    ...
    // Connect
    // hzx 发现addrConnect之前没有建立连接,在这里建立连接
...
void CConnman::DisconnectNodes()
{
    {
        LOCK(cs_vNodes);
        // hzx 监测到网络处于非活跃状态,设置与其他节点断开连接逻辑为真
        if (!fNetworkActive) {
...
// 这个线程持续监视interruptNet线程是否挂了,否则
// 则断开连接
void CConnman::ThreadSocketHandler()
{
    while (!interruptNet) {
        // 监测网络是否活跃,如果不活跃,则与多余节点断开连接
        DisconnectNodes();
        // 通知节点连接状态变化
...
/**
 * hzx 只有非常急需其他seed的时候依赖种子节点
 * 一般情况尽量少发送请求,一方面是为了减少对种子节点的以来,
 * 另外一方面是为了减少种子节点的流量拥塞.
**/
void CConnman::ThreadDNSAddressSeed()
{
    ...
    // hzx VSeeds在chainparams中创建创世块的时候生成,添加了8个可用服务器地址
    ...
    for (const std::string& seed : vSeeds) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, nMaxIPs, true)) {
                ...
                // hzx 解析失败后加入到OneShot中,即之前发送的那个DNS seed不支持相关查询,所以没有解析结果
                AddOneShot(seed);
            }
        }
    }
...
void CConnman::ThreadOpenConnections(const std::vector<std::string> connect)
{
    ...
    // hzx 如果-connect指定了 一些节点, 先于这些节点建立连接,加入VNodes中
    if (!connect.empty()) {
    ...
    // hzx 循环停止的条件是,除非客户端发出停止指令,否则持续添加节点
    while (!interruptNet) {
        ProcessOneShot();
...
bool CConnman::InitBinds(const std::vector<CService>& binds, const std::vector<NetWhitebindPermissions>& whiteBinds)
{
    ...
    // hzx 初始化设置是binds和whiteBinds都是空,把0地址绑定
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    // hzx 从本地存储中读取可用节点信息,存入addrman中
    int64_t nStart = GetTimeMillis();
    {
        CAddrDB adb;
        // hzx 读取peer addr信息,addrman管理peer相关addr信息
        if (adb.Read(addrman))
    ...
    //
    assert(m_msgproc);
    // hzx ,没看懂InterruptSocket5, 将interruptSocks5Recv设置为false,不知道有什么卵用
    InterruptSocks5(false);
    // hzx 重设网络变量, 将其flag 设置为false
    interruptNet.reset();
    ...
    if (!gArgs.GetBoolArg("-dnsseed", true))
        ...
        // ThreadDNSAddressSeed 向DNS节点询问请求相关可用节点,然后加入到addrman中
        threadDNSAddressSeed = std::thread(&TraceThread<std::function<void()>>, "dnsseed", std::function<void()>(std::bind(&CConnman::ThreadDNSAddressSeed, this)));

    // Initiate outbound connections from -addnode
    // 处理-addnode 添加的作为outbound的节点, 如果初始化没有指定addnode参数,同时没有通过rpc增加额外的节点,这个线程可以省略.
